...
export default function VerifyPage({params}:{params:Promise<{serial:string}>}){
 ...
 useEffect(()=>{params.then(p=>{setSerial(p.serial);fetch(`/api/v1/verify/${encodeURIComponent(p.serial)}`).then(r=>r.json()).then(setResult).finally(()=>setLoading(false));});},[params]);
